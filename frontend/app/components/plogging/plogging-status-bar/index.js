...
import { useWindowDimensions, Text, View, StyleSheet, AppState } from "react-native";
...
const PloggingStatusBar = ({ mm = 0, ss = 0, distSumLog, isLogDetail, isPlogging, setTimeSum, timeSumString, setIsSave, setDistSum, handleShowEndPage, saveLogs, islogDetail, logDetailWeather }) => {
  ...
  const setTimeDiff= async () => {
    await AsyncStorage.getItem('startedTime').then(res => {
      if (res) {
        const curr = new Date();
        ...
        const arr = res.split('-');
        const startedTime = new Date(arr[0], arr[1], arr[2], arr[3], arr[4], arr[5]);
        let second = Math.floor((kr_curr.getTime() - startedTime.getTime())/1000);
        console.log(second);
